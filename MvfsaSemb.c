/* Versão 2.0 - Inversão dos parâmetros do CRS zero offset (RN, RNIP, BETA) utilizando o VFSA e Semblance

-Esta é uma versão modificada do programa 'sfvfsa'. 
'sfvfsaSemb' utiliza o cubo de dados como entrada, ao invés da superfície
de tempo de trânsito modelada. Isto permite que este programa seja utilizado para obtenção dos parâmetros do
CRS zero offset em modelos com mais de um refletor. Este programa se aproxima melhor da inversão realizada
na realidade do processamento de dados sísmicos.

A partir de dados sísmicos organizados em famílias CMP (cubo de dados), se
obtém os parâmetros que melhor ajustam uma aproximação de tempo de trânsito CRS 
(Dentre as 11 opções a serem escolhidas) aos dados.
...
O arquivo de entrada padrão 'in.rsf' é o cubo de dados A(m,h,t). Onde:
	x1	é o tempo de trânsito t
	x2	é o meio afastamento h
	x3	é a coordenada do cmp
...
#include "f_vfsaSemb.h" //biblioteca de funções do SFVFSA
...
***************************************************************************************************/
	...
	*/
	...
	sf_warning("nt=%i;dt=%f;ot=%f",nt, dt, ot);
