/* Versão 1.0 - Gerar a superfície de tempo de trânsito CRS zero offset dados (RN, RNIP, BETA)
...
na variável par=, na ordem (RN, RNIP e BETA), que pode ser gerado com sfvfsa.
...
Exemplo de uso:
	...
	<in.rsf sfcrs app=1 m0=5 v0=1.5 verb=1 > out.rsf
...
#include "f_crs.h" // Biblioteca de funções deste programa
...
***************************************************************************************************/
	...
	out = sf_output("out"); // Superfície CRS aproximada
	...
	*/
	...
	if (verb) {
		...
		aproximação CRS está sendo utilizada */
		f_vfsa_aviso(app); 
	}
